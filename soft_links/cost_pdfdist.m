...
LegText{end+1} = 'Tentative preference histogram';
...
LegHandles(end+1) = hLine;
LegText{end+1} = 'Tentative preference PDF';

hold on

%%
% Get data limits to determine plotting range
XLim = [0.8*min(c2), 1*max(c2)];
...
set(hLine,'FaceColor','none','EdgeColor',[0.7 0.7 0.7],...
    'LineStyle','-', 'LineWidth',1);
xlabel('Aggregated preference of measures','FontSize',16)
ylabel('PDF','FontSize',20)
LegHandles(end+1) = hLine;
LegText{end+1} = 'Permanent preference histogram';
bx = gca;
bx.FontSize = 20; 
...
hLine = plot(XGrid,YPlot,'Color',[0.7 0.7 0.7],...
    ...
    'Marker','none', 'MarkerSize',6);
LegHandles(end+1) = hLine;
LegText{end+1} = 'Permanent preference PDF';
