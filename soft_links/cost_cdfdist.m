...
LegText{end+1} = 'Tentative preference CDF';
...
if length(a)>0
    a=a(1);
    text(XGrid(a),0.96,1,strcat('preference',num2str(round(XGrid(a)))),'Color','red','FontSize',14);
...
xlabel('Aggregated preference','FontSize',16)
...
hLine = plot(XGrid,YPlot2,'Color',[0.7 0.7 0.7],...
    ...
    'Marker','none', 'MarkerSize',6);
LegHandles(end+1) = hLine;
LegText{end+1} = 'Permanent preference CDF';
% Adjust figure
box on;
grid on;
hold off;
